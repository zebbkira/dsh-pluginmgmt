...
function prettySource(spec, entryOwner, entryRepo) {
...
function checkUpdates() {
	return withLock(async () => {
		...
		for (const [name, entry] of Object.entries(registry.plugins)) {
			if (entry.owner === void 0 || entry.repo === void 0) continue;
			const latest = await resolveCommit(gitUrl({
				host: "github",
				owner: entry.owner,
				repo: entry.repo
			}), entry.branch);
			if (latest.commit !== void 0 && latest.commit !== entry.ref) result[name] = latest.commit;
		}
		return result;
	});
}
